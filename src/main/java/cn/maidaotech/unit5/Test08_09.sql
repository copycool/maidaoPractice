...
#e
SELECT `name` '名字',SUM(score)'总成绩' FROM score LEFT join students on score.sno=students.sno  WHERE class_id=1  GROUP BY students.`name` desc 
#f
SELECT*FROM score LEFT JOIN students on  score.sno=students.sno WHERE score>80;
#g
