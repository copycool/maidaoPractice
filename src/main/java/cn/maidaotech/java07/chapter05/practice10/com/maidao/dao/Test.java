...
import com.maidao.dao.util.JdbcUtil;
...
//                "s.sno = 10000");
        //e
//        JdbcUtil.executeSelect("SELECT SUM(sc.score) AS \"总分\",s.`first_name` AS \"学生姓名\"\n" +
//                "FROM `score` AS sc\n" +
...
//                "\t\t\t\t\t\t AND sc.score < 60)");
        //h
//        JdbcUtil.executeSelect("SELECT AVG(sc.score) AS \"平均分\",sz.`name` AS \"班级名称\"\n" +
//                "FROM score AS sc\n" +
...
//                "DESC");
        //i
//        JdbcUtil.executeSelect("SELECT MAX(sc.score) AS \"最高分\",MIN(sc.score) AS \"最低分\",c.`name` AS \"课程名称\"\n" +
//                "FROM `score` sc\n" +
//                "LEFT JOIN `course` c\n" +
//                "ON sc.cno = c.cno\n" +
//                "GROUP BY sc.cno\n" +
