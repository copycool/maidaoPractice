...
//public class OrderService implements OrderServiceInterface{
	...
	public Order creatOrder(Account account, Order order) {
		Product product = findIdByName(order.getGoodsPurchased());
		if (product == null) {
            System.out.println(order.getGoodsPurchased() + "不在库存中");
            return null;
        }
		return null;
		Order isorder = checkOrder(order.getGoodsPurchased(), order.getGoodsPurchased(), product, account);
        if (isorder == null) {
	...
	private Order checkOrder(String goodsPurchased, String goodsPurchased2, Product product, Account account) {
	...
	private Product findIdByName(Object productName) {
		return null;
	}*/
